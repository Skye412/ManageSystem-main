...
const secretKey = process.env.JWT_SECRET || config.secretKey;

const refreshSecretKey = process.env.JWT_REFRESH_SECRET || config.refreshSecretKey;
//验证访问令牌中间件
...
const authenticateRefreshToken = async (req, res, next) => {
  try {
    ...
    if (storedToken !== refreshToken) {
      return res.status(401).json({ code: 401, message: '刷新令牌已失效' }); 
    }
    req.user = decoded;
    next();
  } catch (err) {
