...
const storage = multer.diskStorage({
    destination: function (req, file, cb) {
        ...
        const dir = path.join(__dirname, '../../files/resumes', safeId);
...
const uploadResume = (req, res) => {
    upload.single('resume')(req, res, async (err) => {
        ...
        try {
            ...
            res.status(200).json({ code: 200, message: '简历上传成功', filePath: formattedPath });
        } catch (error) {
            console.error('数据库更新失败:', error);
            res.status(500).json({ code: 500, message: '数据库更新失败' });
        }
    });
};
