...
async function getMajorsAndClasses(req, res) {
    try {
        ...
        res.status(500).json({ code: 500, message: "查询专业及班级失败" });
    }
}
...
async function getStudentsByClassName(req, res) {
    try {
        ...
        if (!className) {
            return res.status(400).json({ code: 400, message: "班级名不能为空" });
        }
        ...
        res.status(500).json({ code: 500, message: "查询学生失败" });
...
async function addMajorClass(req, res) {
    try {
        ...
        if (!major_name || !class_name || !class_advisor) {
            return res.status(400).json({ code: 400, message: "专业名、班级名和班主任不能为空" });
        }
        ...
        if (existingClass.length > 0) {
            ...
            return res.status(409).json({ code: 409, message: "专业班级已存在" });
        }
        ...
        // 异常处理：事务回滚并记录错误
        await executeQuery("ROLLBACK");
        res.status(500).json({ code: 500, message: "添加专业班级失败" });
        throw new Error("服务器内部错误，请稍后重试");
    }
}
...
async function addStudentToClass(req, res) {
    try {
        let { student_id, name, gender, age, major, className, graduation_year, status } = req.body;
        if (!age) {
            age = 22;
        }
        if (!graduation_year) {
        ...
        if (!student_id || !name || !gender || !major || !className) {
            return res.status(400).json({ code: 400, message: "学生ID、姓名、性别、专业和班级不能为空" });
        }
        ...
        if (existingStudent.length > 0) {
            await executeQuery("ROLLBACK");
            return res.status(409).json({ code: 409, message: "学生已存在" });
        }

        // 添加学生（密码默认为 '123456' 的 bcrypt 哈希）
        const bcrypt = require('bcrypt');
        const hashedPassword = await bcrypt.hash('123456', 10);
        const insertStudentSql = `
            INSERT INTO students (student_id, password, name, gender, age, major, class, graduation_year, status)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        `;
        await executeQuery(insertStudentSql, [student_id, hashedPassword, name, gender, age, major, className, graduation_year, status]);
        ...
        // 出错时回滚事务
        await executeQuery("ROLLBACK");
        res.status(500).json({ code: 500, message: "添加学生失败" });
...
async function addEnterprise(req, res) {
    try {
        ...
        if (!name || !contact_name || !address || !phone || !email) {
            return res.status(400).json({ code: 400, message: "企业名称、联系人、地址、电话和邮箱不能为空" });
        }
        ...
        if (existingEnterprise.length > 0) {
            return res.status(409).json({ code: 409, message: "企业已存在" });
        }

        // 执行数据插入：新增企业记录（密码默认为 '123456' 的 bcrypt 哈希）
        const bcrypt = require('bcrypt');
        const hashedPassword = await bcrypt.hash('123456', 10);
        const sql = `
            INSERT INTO enterprises (name, password, contact_name, address, phone, email)
            VALUES (?, ?, ?, ?, ?, ?)
        `;
        await executeQuery(sql, [name, hashedPassword, contact_name, addressStr, phone, email]);
...
async function getAllEnterprises(req, res) {
    try {
        ...
        const countSql = `SELECT COUNT(*) as total FROM enterprises`;
        const countResult = await executeQuery(countSql);
        const total = countResult[0].total;

        const sql = `SELECT * FROM enterprises ORDER BY created_at DESC LIMIT ?, ?`;
        const results = await executeQuery(sql, [offset, pageSize]);
        ...
        res.status(500).json({ code: 500, message: "查询企业信息失败" });
        throw new Error("服务器内部错误，请稍后重试");
    }
}
...
async function addPosition(req, res) {
  try {
    ...
    if (!position_name || !industry || !description || !salary_range || !work_address || !deadline || !enterprise_name) {
      return res.status(400).json({ code: 400, message: "职位名称、所属行业、职位描述、薪资范围、工作地址、截至日期和所属企业不能为空" });
    }
    ...
    if (existingPosition.length > 0) {
      return res.status(409).json({ code: 409, message: "职位已存在" });
    }
    ...
    if (insertResult.affectedRows === 0) {
      return res.status(400).json({ code: 400, message: "添加职位失败，请稍后重试" });
    }
    ...
    res.status(400).json({ code: 400, message: "服务器内部错误，请稍后重试" });
  }
}
...
async function getPositions(req, res) {
    try {
      ...
      const countSql = `SELECT COUNT(*) as total FROM positions`;
      const countResult = await executeQuery(countSql);
      const total = countResult[0].total;
      ...
      res.status(500).json({ code: 500, message: "查询职位信息失败" });
      throw new Error("服务器内部错误，请稍后重试");
    }
  }
...
async function deletePosition(req, res) {
  try {
    const { id } = req.body;
    if (!id) {
      return res.status(400).json({ code: 400, message: "职位ID不能为空" });
    }
...
async function getEmploymentStatistics(req, res) {
    try {
        ...
        res.status(500).json({ code: 500, message: "查询就业统计数据失败" });
        throw new Error("服务器内部错误，请稍后重试");
    }
}
...
async function addNotice(req, res) {
  try {
    ...
    if (!title || !content || !admin_id) {
      return res.status(400).json({ code: 400, message: "标题、内容和管理员ID不能为空" });
    }
    ...
    res.status(500).json({ code: 500, message: "新增公告失败" });
...
async function getNotices(req, res) {
  try {
    ...
    const countSql = `SELECT COUNT(*) as total FROM notices`;
    const countResult = await executeQuery(countSql);
    const total = countResult[0].total;

    const sql = `SELECT * FROM notices ORDER BY created_at DESC LIMIT ?, ?`;
    const results = await executeQuery(sql, [offset, pageSize]);
    res.status(200).json({ code: 200,
      message: "查询成功",
      data: results,
    ...
    res.status(500).json({ code: 500, message: "查询公告失败" });
...
async function getNoticeDetail(req, res) {
  try {
    ...
    res.status(500).json({ code: 500, message: "查询公告详情失败" });
...
async function reviewPosition(req, res) {
  try {
    ...
    if (!id || !status) {
      return res.status(400).json({ code: 400, message: "职位ID和审核状态不能为空" });
    }
...
async function getPositionDetail(req, res) {
  try {
    ...
    res.status(500).json({ code: 500, message: "查询职位详情失败" });
