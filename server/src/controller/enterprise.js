...
const getDeliveriesByEnterpriseId = async (req, res) => {
  ...
  try {
    ...
    const countSql = `SELECT COUNT(*) as total FROM resume_deliveries WHERE enterprise_id = ?`;
    const countResult = await executeQuery(countSql, [enterpriseId]);
    ...
    const sql = `
      ...
      LIMIT ?, ?
    `;
    const results = await executeQuery(sql, [enterpriseId, offset, pageSize]);
    if (results.length === 0) {
      return res.status(200).json({ code: 200,
        message: "未找到相关投递信息",
        data: [],
...
const acceptCandidate = async (req, res) => {
  ...
  try {
    ...
    await executeQuery(sql, [studentId]);
    res.status(200).json({ code: 200, message: "已录用该学生" });
  } catch (error) {
...
const rejectCandidate = async (req, res) => {
  ...
  try {
    ...
    await executeQuery(sql, [studentId]);
    res.status(200).json({ code: 200, message: "已拒绝该学生" });
  } catch (error) {
...
const markAsViewed = async (req, res) => {
  ...
  try {
    ...
    await executeQuery(sql, [studentId]);
    res.status(200).json({ code: 200, message: "已标记为已查看" });
  } catch (error) {
