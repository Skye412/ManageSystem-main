...
async function getStudentInfo(req, res) {
    try {
        ...
        res.status(500).json({ code: 500, message: "查询学生信息失败" });
    }
}
...
async function submitResume(req, res) {
  try {
    ...
    if (checkDeliveryResult.length > 0) {
      return res.status(200).json({ code: 200, message: "您已经投递过该职位" });
    }
    ...
    res.status(500).json({ code: 500, message: "简历投递失败，请稍后重试" });
  }
}
...
async function getResumeDeliveries(req, res) {
  try {
    ...
    res.status(500).json({ code: 500, message: "获取简历投递信息失败，请稍后重试" });
  }
}
