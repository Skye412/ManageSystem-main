...
async function studentSendMessage(req, res) {
    try {
        ...
        if (!enterprise_id || !message) {
            return res.status(400).json({ code: 400, message: "企业ID和消息内容不能为空" });
        }
        if (!student_id && !admin_id) {
            return res.status(200).json({ code: 200, message: "必须指定接收方（学生或管理员）" });
        }
        ...
        res.status(500).json({ code: 500, message: "发送消息失败，请稍后重试" });
    }
}
...
async function getmessageslist(req, res) {
    try {
        ...
        // 参数校验
        if (params.length !== 1) {
            return res.status(200).json({ code: 200, message: "必须且只能传入一个ID参数" });
        }
        ...
        if (!results || results.length === 0) {
            return res.status(200).json({ code: 200, message: "没有找到相关聊天记录" });
        }
...
async function getChatMessages(req, res) {
    try {
        ...
        if (!student_id || !enterprise_id) {
            return res.status(200).json({ code: 200, message: "必须同时传入学生ID和企业ID" });
        }
        ...
        if (!studentInfo || !enterpriseInfo) {
            return res.status(200).json({ code: 200, message: "未找到相关用户信息" });
        }
