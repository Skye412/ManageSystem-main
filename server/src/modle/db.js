...
const initIndexes = async () => {
  const indexes = [
    ...
    "CREATE INDEX idx_chat_enterprise ON chat_messages(enterprise_id)",
    "CREATE INDEX idx_chat_admin ON chat_messages(admin_id)",
    "CREATE INDEX idx_chat_sent_at ON chat_messages(sent_at)",
    // resume_deliveries 索引
    "CREATE INDEX idx_delivery_student ON resume_deliveries(student_id)",
    "CREATE INDEX idx_delivery_position ON resume_deliveries(position_id)",
    "CREATE INDEX idx_delivery_enterprise ON resume_deliveries(enterprise_id)",
    // positions 索引
    "CREATE INDEX idx_position_enterprise ON positions(enterprise_id)",
  ...
  for (const sql of indexes) {
    try {
      await new Promise((resolve, reject) => {
        db.query(sql, (error) => {
          if (error && error.code !== 'ER_DUP_KEYNAME') reject(error);
          else resolve();
        });
      });
    } catch (err) {
