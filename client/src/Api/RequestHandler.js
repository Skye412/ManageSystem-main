 ...
 */
export async function sendRequest(url, method = 'GET', requiresAuth = false,userType = 'student', data = null,  config = {}) {
  try {
    // 如需授权，设置令牌
    if (requiresAuth) {
      let store;
      switch(userType) {
        case 'manage':
          const mamage = manageStore();
          store = mamage.manageInfo;
          break;
        case 'enterprise':
          const enterprise = enterpriseStore();
          store = enterprise.enterpriseInfo;
          break;
        default:
          const student = studentStore();
          store = student.studentInfo;
      }
      ...
      Axiosinstance.defaults.headers.common['Authorization'] = `Bearer ${store.accessToken}`;
    }
